...
async function loadLine(url, lineName, color, info, canvasWidth, canvasHeight) {
  ...
  const lineData = { points, color, name: lineName, info: info };
  // 缓存线路数据
  lineCache[url] = lineData;
  return lineData;
}
  ...
  useEffect(() => {
    ...
    const fetchAllLines = async () => {
      ...
      if (lines[currentLine] === undefined) {
        setCurrentLineInfo("");
        setCurrentLineName("");
        setCurrentLineColor("transparent");
      } else {
        setCurrentLineInfo(lines[currentLine].info);
        setCurrentLineName(lines[currentLine].name);
        setCurrentLineColor(lines[currentLine].color);
      }
    };
    ...
    const handleMouseMove = (e) => {
      ...
      if (hoveredLineIndex !== -1) {
        ...
        if (lines[hoveredLineIndex] === undefined) {
          setCurrentLineInfo("");
          setCurrentLineName("");
          setCurrentLineColor("transparent");
        } else {
